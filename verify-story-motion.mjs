...
for(let i=0;i<sceneCount;i++){
 ...
 for(let j=0;j<sceneCount;j++)if(j!==i)assert(scenes[j].inert);
}
context.scrollY=story.offsetTop+story.offsetHeight-900;listeners.scroll();settle();
assert.equal(scenes.at(-1).style.opacity,'1','Final contact chapter disappears at the bottom');
...
context.window.PortfolioModalOpen=true;const before=scenes.at(-1).style.opacity;context.scrollY=0;listeners.scroll();settle();assert.equal(scenes.at(-1).style.opacity,before,'Story moves behind an open project');
