...
(function () {
  ...
  function sampleSource(now) {
    ...
    try {
      // Fill every cell from the same centred cover crop as the video.
      // No lateral offset and no empty strip down the left edge.
      octx.drawImage(src, sx, sy, cw, ch, 0, 0, COLS, ROWS);
  ...
  function frame(now) {
    ...
    const pp = reduceMotion ? 1 : (window.DigitStory ? window.DigitStory.timeline(pS) : pS);
    const storyMix = reduceMotion ? 0 : ss(.12,.155,pS)*(1-ss(.61,.68,pS));
    window.DigitStory?.update(reduceMotion?1:pS);
    ...
    const handoff = window.DigitStory?.reveal(pS) ?? ss(0.18,0.78,pp);
    const videoReveal = reduceMotion ? 1 : (window.DigitStory ? handoff : ss(0.18,0.78,pp)*(1-storyMix));
    const digitPresence = reduceMotion ? 0 : (window.DigitStory ? 1-handoff : Math.max(storyMix,1-ss(0.22,0.88,dp)));
    const tide = ss(0.15, 0.92, dp)*(1-storyMix);
    const scatter = ss(0.30, 1.0, dp)*(1-storyMix);
    ...
    // The original video stays centred and full-bleed throughout the
    // opening. The digit sampling above uses this exact same crop.
    if (media) {
    ...
    if (bridging1) {
      const b1 = window.DigitStory ? 0 : pulse(0.27, 0.30, 0.35, 0.38, pp);
      bridging1.style.opacity = b1.toFixed(3);
      bridging1.style.transform = 'translateY(' + ((1 - b1) * 24) + 'px)';
    }
    if (bridging2) {
      const b2 = window.DigitStory ? 0 : pulse(0.38, 0.41, 0.46, 0.49, pp);
    ...
    portraitBrightness.set(bright);
    const morph=!reduceMotion&&window.DigitMorph?window.DigitMorph.apply(bright,COLS,ROWS,pS,now):null;
    ...
    for (let k = 0; k < n; k++) {
      ...
      const dying = morph ? 0 : 1 - ss(vanishRadius - soft, vanishRadius, cellDist[k]);
      const alive = (1 - dying) * digitPresence;
      const a = alive * (0.14*(1-storyMix) + b * 0.9 + storyMix*.025);
      if (a < 0.012) continue;

      if (!reduceMotion && !(pS>=.12&&pS<=.65)) {
      ...
      let x = (k % COLS) * cellW + cellW / 2 + driftX;
      let y = ((k / COLS) | 0) * cellH + cellH / 2 + driftY;
      if(morph){x=morph.xs[k]*innerWidth;y=morph.ys[k]*innerHeight;}
      if(storyMix&&!morph){
        // Low-signal digits fall; bright digits settle into the sampled silhouette.
        y+=(((now*.035+(k%COLS)*31)%cellH)-cellH/2)*storyMix*(1-b);
        const pointer=window.DigitStory.pointer();
        const dx=x-pointer.x*innerWidth,dy=y-pointer.y*innerHeight,d=Math.hypot(dx,dy);
        const force=Math.max(0,1-d/100)*storyMix*9;
        x+=dx/Math.max(1,d)*force;y+=dy/Math.max(1,d)*force;
      }
      const codeAlphabet='01{}[]<>/;=()+-*:.xyf';
      const glyph=morph?.codeWeight>((k*17)%100)/100?codeAlphabet[(k+Math.floor(now/400))%codeAlphabet.length]:digits[k];
